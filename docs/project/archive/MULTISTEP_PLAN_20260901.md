...
> **历史计划归档说明（2026-09-04）**：本文是 2026-09-01 制定的 detached multi-step
> 工作包 1–6 实施前计划，保留当时的算法、文件计划与运行入口原文。工作包 1–6 已全部
> 执行完毕（其中 full3d K3 按预注册条件阻塞），实际结果以实验 06/10/11 的
> `RESULTS.md` 为准；现行方向文档 `docs/project/CURRENT_CHALLENGES_AND_NEXT_STEPS.md`
> 只保留当前证据、未解决问题与待决策项。文中一切"计划/尚不可执行/未做"表述均为
> 当时状态，不再反映现状。
